...
import { SkeletonCard, EmptyState } from "@/components/history/UI";
  ...
  return (
    <main className="min-h-screen bg-[#F9FAFB]">
      <div className="flex-1 px-4 lg:w-[950px] mx-auto mt-32 space-y-10 pb-20">
        ...
        {/* HEADER SECTION */}
        <Header filterDate={filterDate} setFilterDate={setFilterDate} />

        {/* CONTENT */}
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><SkeletonCard /><SkeletonCard /></div>
        ) : filteredOrders.length === 0 ? (
          <EmptyState filterDate={filterDate} onReset={() => setFilterDate("")} />
        ) : (
          <AnimatePresence>
            {Object.entries(groupedOrders).map(([date, items], idx) => (
              <section key={date} className="space-y-6">
                <div className="flex items-center gap-2">
                  <Calendar size={16} className="text-gray-700" />
                  <h2 className="text-sm font-medium text-gray-700 capitalize">{date}</h2>
                </div>
...
const Header = ({ filterDate, setFilterDate }) => (
  <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-md flex flex-col lg:flex-row justify-between gap-8">
    <div className="space-y-1.5">
      <h1 className="text-3xl font-bold tracking-tighter">Riwayat Pesanan</h1>
      <p className="text-[13px] text-gray-400 font-medium">Pantau aktivitas pengiriman Anda secara real-time.</p>
    </div>
    <div className="flex flex-wrap items-center gap-4">
      <div className="relative">
        <Calendar size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
        <input type="date" value={filterDate} onChange={(e) => setFilterDate(e.target.value)}
          className="pl-11 pr-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-medium focus:ring-2 focus:ring-gray-900/5 outline-none" />
        {filterDate && <button onClick={() => setFilterDate("")} className="absolute right-3 top-1/2 -translate-y-1/2 bg-gray-300 p-2 rounded-full text-gray-500 hover:text-gray-600">
          <X size={16} />
        </button>}
      </div>
    </div>
